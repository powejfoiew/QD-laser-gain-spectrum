...
# ---------------------------------------------------------------
# 2. SIMULATION RUNNER & OBJECTIVE FUNCTION
# ---------------------------------------------------------------
def run_simulation(app, params):
    """
    Writes custom_refbase.mat, runs PICWave simulation, and calculates 
    score based on smooth continuous distance to target occupations.
    """
    ...
    # ---------------------------------------------------------------
    # SMOOTHNESS & OVERSHOOT PENALTY (Squared L2 Norm)
    # ---------------------------------------------------------------
    # 1. Transient Overshoot
    gs_overshoot  = max(0.0, np.max(gs_series) - final_gs)**2
    es1_overshoot = max(0.0, np.max(es1_series) - final_es1)**2
    es2_overshoot = max(0.0, np.max(es2_series) - final_es2)**2

    # 2. Total Variation (TV) Excess (Penalizes oscillations)
    gs_excess_tv  = (np.sum(np.abs(np.diff(gs_series))) - abs(final_gs - gs_series[0]))**2
    es1_excess_tv = (np.sum(np.abs(np.diff(es1_series))) - abs(final_es1 - es1_series[0]))**2
    es2_excess_tv = (np.sum(np.abs(np.diff(es2_series))) - abs(final_es2 - es2_series[0]))**2
    ...
    # ---------------------------------------------------------------
    # STRICT CARRIER OCCUPATION (Squared Mean Error / L2 Norm)
    # ---------------------------------------------------------------
    err_gs  = (final_gs - TARGET_GS)**2
    err_es1 = (final_es1 - TARGET_ES1)**2
    err_es2 = (final_es2 - TARGET_ES2)**2
...
def main():
    ...
    if best_result is not None:
        best_iter, best_params, best_res_dict = best_result
        print("\n=======================================================")
        print(f"OPTIMIZATION COMPLETE - BEST RESULT (Iteration {best_iter})")
        print("=======================================================")
        print(f"Final Densities : GS={best_res_dict['final_gs']:.4f}, ES1={best_res_dict['final_es1']:.4f}, ES2={best_res_dict['final_es2']:.4f}")
        print(f"Final Score     : {best_res_dict['score']:.6f}")
        print("\n--- Optimized Parameter Set ---")
        for k in PARAM_KEYS:
            print(f"'{k}': {best_params[k]:.4e},")
        print("=======================================================\n")
    ...
    plt.title("State Occupation Convergence vs Target Densities", fontsize=12, fontweight='bold')
    plt.xlabel("Iteration", fontsize=10, fontweight='bold')
    plt.ylabel("Carrier Density (10^18 cm^-3)", fontsize=10, fontweight='bold')
    plt.grid(True, linestyle=':', alpha=0.7)
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig("optimisation_occupation.png", dpi=300)

    # -----------------------------------------------------------
    # PLOT 2: BEST RUN TRANSIENT CARRIER DENSITIES
    # -----------------------------------------------------------
    if best_time_series is not None:
        plt.figure(figsize=(10, 6))
        
        time_axis = np.array(best_time_series["gs_carrier_density"][1][1:])
        
        gs_raw  = np.array(best_time_series["gs_carrier_density"][2][1:])
        es1_raw = np.array(best_time_series["es1_carrier_density"][2][1:])
        es2_raw = np.array(best_time_series["es2_carrier_density"][2][1:])
        
        gs_plot  = gs_raw  if gs_raw[-1]  < 1e10 else gs_raw  / 1e18
        es1_plot = es1_raw if es1_raw[-1] < 1e10 else es1_raw / 1e18
        es2_plot = es2_raw if es2_raw[-1] < 1e10 else es2_raw / 1e18
        
        plt.plot(time_axis, gs_plot, 'b-', linewidth=2, label=f'GS (Best Run)')
        plt.plot(time_axis, es1_plot, 'orange', linewidth=2, label=f'ES1 (Best Run)')
        plt.plot(time_axis, es2_plot, 'g-', linewidth=2, label=f'ES2 (Best Run)')
        
        plt.axhline(TARGET_GS, color='b', linestyle='--', alpha=0.5, label='GS Target')
        plt.axhline(TARGET_ES1, color='orange', linestyle='--', alpha=0.5, label='ES1 Target')
        plt.axhline(TARGET_ES2, color='g', linestyle='--', alpha=0.5, label='ES2 Target')
        
        plt.title("Best Optimization Run: Carrier Densities over Time", fontsize=12, fontweight='bold')
        plt.xlabel("Time (s)", fontsize=10, fontweight='bold')
        plt.ylabel("Carrier Density (10^18 cm^-3)", fontsize=10, fontweight='bold')
        plt.grid(True, linestyle=':', alpha=0.7)
        plt.legend(loc='best')
        plt.tight_layout()
        plt.savefig("best_run_densities.png", dpi=300)
