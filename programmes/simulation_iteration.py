...
def run_simulation(app, params):
    ...
    # Objective function:
    # 1. Primary: maximize log10 of power (high power lasing)
    # 2. Primary: bonus for sustained lasing duration (> 0.1 s)
    # 3. Secondary: occupation close to 0.2
    power_score = np.log10(max(max_power, 1e-10))
    duration_score = min(lasing_duration / 0.1, 1.0) * 10.0
    occ_penalty = -20.0 * abs(final_gs - 0.2)

    score = power_score + duration_score + occ_penalty
...
# ---------------------------------------------------------------
# 3. OPTIMIZATION CONTROLLER (300 ITERATIONS OVERNIGHT RUN)
# ---------------------------------------------------------------
...
def load_previous_best(csv_path="optimisation_results.csv"):
    """
    ...
    try:
        ...
        # Sort by score or final_gs to pick best starting point
...
def main():
    MAX_NEW_ITERATIONS = 300 # Set for 300-iteration overnight run
    LHS_SAMPLES = 40

    prev_df, start_vec = load_previous_best("optimisation_results.csv")
    ...
    def evaluate_vector(log2_vec):
        ...
        if res["crashed"]:
            ...
            print(f"Status: SUCCESS | Final GS Occ: {res['final_gs']:.4f} | Max Power: {res['max_power']:.2f} | Lasing Dur: {res['lasing_duration']:.4f}s | Score: {res['score']:.4f}")
        ...
        # Save CSV progressively in case of interruption
        pd.DataFrame(history).to_csv("optimisation_results.csv", index=False)
    ...
    print("Full results updated in optimisation_results.csv")
    ...
    if not valid_df.empty:
        ...
        print(f"  Final GS Carrier Density : {best_row['final_gs']:.4f}")
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(df.loc[valid_mask, 'iteration'], df.loc[valid_mask, 'final_gs'], 'mo-')
    plt.axhline(0.2, color='r', linestyle='--', label='Target GS Occupation (0.2)')
    plt.title("Final GS Occupation vs Iteration")
    plt.xlabel("Iteration")
    plt.ylabel("Final GS Carrier Density / Occupation")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
