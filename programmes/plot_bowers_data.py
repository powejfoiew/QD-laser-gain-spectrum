...
# ── Figure sizing (similar to MATLAB script) ──────────────────────
# MATLAB uses [200, 200, 670, 450] and [200, 200, 450, 200]
# We'll use nice sizes in inches (e.g., 6.7 in x 4.5 in, and 4.5 in x 2.0 in)
...
# Region 1: 1e3 to 1e4
#ax1.fill_between([1e3, 1e8], [1e3, 1e3], [1e4, 1e4], color=colors[0], edgecolor=colors[0], label='_nolegend_')
# Region 2: 1e4 to 1e5
#ax1.fill_between([1e3, 1e8], [1e4, 1e4], [1e5, 1e5], color=colors[2], edgecolor=colors[2], label='_nolegend_')

# Plot experimental data
#ax1.plot(pn[0]['x'], pn[0]['y'], color=colors[8], linewidth=1, label="1000 $\mu$m")
...
ax1.axvspan(1e3,    1.25e5, color='#EE3377', alpha=0.4, label='Low Freq (f^-1)')
ax1.axvspan(1.25e5, 1e7,    color='#CC6677',  alpha=0.3, label='Mid Freq (f^-2)')
...
ax1.plot(blue_xs, y_blue_fit,   color='#882255', label='Blue noise (~1/f^2)',linestyle='dashed')
...
#fig1.savefig(os.path.join(folder, "QD_Frequency_noise.png"), dpi=300)
#fig1.savefig(os.path.join(folder, "QD_Frequency_noise.pdf"))
